...
DATABASES = {
    "default": {
        ...
        "CONN_HEALTH_CHECKS": db_backend
        != "sqlite3",  # Will only be used from Django 4.1 onwards
        "OPTIONS": db_options,
        "TEST": {},
    }
}
...
PRETIX_PLUGINS_SHOW_META = config.getboolean(
    "pretix", "plugins_show_meta", fallback=True
)
...
MAIL_CUSTOM_SENDER_VERIFICATION_REQUIRED = config.getboolean(
    "mail", "custom_sender_verification_required", fallback=True
)
MAIL_CUSTOM_SENDER_SPF_STRING = config.get(
    "mail", "custom_sender_spf_string", fallback=""
)
MAIL_CUSTOM_SMTP_ALLOW_PRIVATE_NETWORKS = config.getboolean(
    "mail", "custom_smtp_allow_private_networks", fallback=DEBUG
)
...
if HAS_REDIS:
    ...
    if USE_REDIS_SENTINEL:
        ...
        OPTIONS["SENTINELS"] = [
            tuple(sentinel) for sentinel in loads(config.get("redis", "sentinels"))
        ]
...
HAS_CELERY_BROKER_TRANSPORT_OPTS = config.has_option(
    "celery", "broker_transport_options"
)
HAS_CELERY_BACKEND_TRANSPORT_OPTS = config.has_option(
...
if HAS_CELERY:
    ...
    if HAS_CELERY_BROKER_TRANSPORT_OPTS:
        CELERY_BROKER_TRANSPORT_OPTIONS = loads(
            config.get("celery", "broker_transport_options")
        )
    if HAS_CELERY_BACKEND_TRANSPORT_OPTS:
        CELERY_RESULT_BACKEND_TRANSPORT_OPTIONS = loads(
            config.get("celery", "backend_transport_options")
        )
else:
...
LANGUAGES = []
LANGUAGES_ENABLED = [
    lang for lang in config.get("languages", "enabled", fallback="").split(",") if lang
]
...
LOGGING = {
    ...
    "filters": {
        ...
        "request_id": {"()": "pretix.helpers.logs.RequestIdFilter"},
    },
    "handlers": {
        "console": {
            "level": loglevel,
...
CELERY_TASK_ROUTES = (
    [
        ...
        (
            "pretix.base.services.export.scheduled_organizer_export",
            {"queue": "background"},
        ),
...
FILE_UPLOAD_MAX_SIZE_IMAGE = (
    1024 * 1024 * config.getint("pretix_file_upload", "max_size_image", fallback=10)
)
FILE_UPLOAD_MAX_SIZE_FAVICON = (
    1024 * 1024 * config.getint("pretix_file_upload", "max_size_favicon", fallback=1)
)
FILE_UPLOAD_MAX_SIZE_EMAIL_ATTACHMENT = (
    1024
    * 1024
    * config.getint("pretix_file_upload", "max_size_email_attachment", fallback=5)
)
FILE_UPLOAD_MAX_SIZE_EMAIL_AUTO_ATTACHMENT = (
    1024
    * 1024
    * config.getint("pretix_file_upload", "max_size_email_auto_attachment", fallback=1)
)
FILE_UPLOAD_MAX_SIZE_OTHER = (
    1024 * 1024 * config.getint("pretix_file_upload", "max_size_other", fallback=10)
)
